...
public class ArmoryRoomPopulator extends MazeRoomBlockPopulator {
        ...
        for(int x2 = x; x2 <= x + 7; x2 += 1)
            for(int y2 = y; y2 <= y + 5; y2++)
                for(int z2 = z; z2 <= z + 7; z2 += 1)
                    setGeneratedBlock(chunk.getBlock(x2, y2 + floorOffset, z2), Material.COBBLESTONE);

        // Bedrock
        for(int x2 = x + 1; x2 <= x + 6; x2 += 1)
            for(int y2 = y; y2 <= y + 4; y2++)
                for(int z2 = z + 1; z2 <= z + 6; z2 += 1)
                    setGeneratedBlock(chunk.getBlock(x2, y2 + floorOffset, z2), Material.BEDROCK);

        // Air
        for(int x2 = x + 2; x2 <= x + 5; x2 += 1)
            for(int y2 = y + 1; y2 <= y + 3; y2++)
                for(int z2 = z + 2; z2 <= z + 5; z2 += 1)
                    setGeneratedBlock(chunk.getBlock(x2, y2 + floorOffset, z2), Material.AIR);
        for(int x2 = x + 1; x2 <= x + 6; x2 += 1)
            for(int y2 = y + 1; y2 <= y + 5; y2++)
                setGeneratedBlock(chunk.getBlock(x2, y2 + floorOffset, z), Material.AIR);

        // Pumpkins
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 1, z + 2), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 1, z + 5), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 1, z + 2), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 1, z + 5), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 2, z + 2), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 2, z + 5), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 2, z + 2), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 2, z + 5), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 3, z + 2), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 3, z + 5), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 3, z + 2), Material.JACK_O_LANTERN);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 3, z + 5), Material.JACK_O_LANTERN);

        // Chests
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 1, z + 3), Material.CHEST);
        setGeneratedBlock(chunk.getBlock(x + 2, yFloor + 1, z + 4), Material.CHEST);
        setGeneratedBlock(chunk.getBlock(x + 3, yFloor + 1, z + 5), Material.CHEST);
        setGeneratedBlock(chunk.getBlock(x + 4, yFloor + 1, z + 5), Material.CHEST);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 1, z + 3), Material.CHEST);
        setGeneratedBlock(chunk.getBlock(x + 5, yFloor + 1, z + 4), Material.CHEST);

        // Iron doors
        MaterialUtils.setDoorHalf(chunk.getBlock(x + 3, yFloor + 1, z + 1), Material.IRON_DOOR, BlockFace.SOUTH, Bisected.Half.BOTTOM, Door.Hinge.LEFT, false);
        MaterialUtils.setDoorHalf(chunk.getBlock(x + 3, yFloor + 2, z + 1), Material.IRON_DOOR, BlockFace.SOUTH, Bisected.Half.TOP, Door.Hinge.LEFT, false);
        MaterialUtils.setDoorHalf(chunk.getBlock(x + 4, yFloor + 1, z + 1), Material.IRON_DOOR, BlockFace.SOUTH, Bisected.Half.BOTTOM, Door.Hinge.RIGHT, false);
        MaterialUtils.setDoorHalf(chunk.getBlock(x + 4, yFloor + 2, z + 1), Material.IRON_DOOR, BlockFace.SOUTH, Bisected.Half.TOP, Door.Hinge.RIGHT, false);
	}
	...
	public void addItemsToChest(Random random, Chest chest) {
		...
		if(random.nextInt(100) < 20)
			items.add(MaterialUtils.createItemStack(Material.RED_SANDSTONE_STAIRS, 7, (short) 0));
		if(random.nextInt(100) < 20)
